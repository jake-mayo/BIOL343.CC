...
1. **Allie Goodyear** - 20443234
    +[BIOL343_CC](https://github.com/alliegoodyear/BIOL343_CC)
2. **Coralie Spakowski** - 20471806
    +[BIOL343_CC](https://github.com/coraliespakowski/BIOL343_CC)
3. **Zeshan Ghuman** - 20479122
    +[BIOL343_CC](https://github.com/Potato9715/BIOL343_CC)
4. **Jake Mayo** - 20186062
    +[BIOL343_CC](https://github.com/jake-mayo/BIOL343.CC)
5. **Noah Lu** - 20483156
    +[BIOL343_CC](https://github.com/NLRcode/BIOL343_CC)
6. **Iryna Oz** - 20508451
    +[BIOL343_CC](https://github.com/irynaoz323/BIOL343_CC)
...
|-----------------|-----------------|-----------------|
| population type     | Indicates whether the population is asexual or sexual  | No units     |
| ovary width | A measure of the width of the ovaries which is a reliable measure of overall flower size    | mm     |
| ovary height     | A measure of the height of the ovaries which is a reliable measure of overall flower size     | mm     |
| flower number     | The average number of flowers produced by each individual in a sample of plants from each population     | Count     |
...
names(cc1_data) <- c("population_type","population_name","latitude","longitude","ovary_width","ovary_height","average_august_temperature","flower_number")
ncol(cc1_data)
names(cc1_data)

cc1_data <- cc1_data %>% mutate(average_august_temperature = as.numeric(average_august_temperature))
...
Ovary witdth and height are measured in mm and ovary area is measured in mm$^$
...
```{r area-table, include=F}
kable(area_means, caption = "Summary Table")
```
...
ggplot(aes(x=ovary_area), data=cc1_data) + 
    labs(title="Population Type vs Ovary Area in Solanum pimpinellifolium", colour =NULL) +
    geom_histogram(binwidth=1) +
    facet_wrap(.~population_type)+
    ylab("Ovary Area")+
    xlab(expression(mm^2))+
    theme_classic()
```
...
![Source: https://onq.queensu.ca/d2l/le/dropbox/1208667/492096/DownloadAttachment?fid=54136518](./images/Hydroponic_g13.jpg)
